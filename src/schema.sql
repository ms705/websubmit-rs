...
-- this was not used QUERY leclist: SELECT lectures.id, lectures.label, submitted.`user`, lec_qcount.qcount, submitted.num_answered FROM lectures LEFT JOIN lec_qcount ON (lectures.id = lec_qcount.lec) LEFT JOIN submitted ON (lectures.id = submitted.lec) WHERE submitted.`user` = ?;
